...
%-------------------------------------------------------------------------- 
%%
function res=PCIe_9852_2CH_INIT_(channel)
    %clc %clear console
    %clear all;
    %close all;
...
%     clear d;
    ...
    error = calllib(LIB,'WD_AI_CH_Config',card,0,AdRange);
    if error < 0
        calllib ('dasklib','WD_Release_Card',card);
        unloadlibrary(LIB);
    ...
    error = calllib(LIB,'WD_AI_CH_ChangeParam',card,0,P9852_AI_IMPEDANCE, Impedance);
    if error < 0
        calllib ('dasklib','WD_Release_Card',card);
        unloadlibrary(LIB);
    ...
    error = calllib(LIB,'WD_AI_CH_ChangeParam',card,1,P9852_AI_IMPEDANCE, Impedance);
    if error < 0
        calllib ('dasklib','WD_Release_Card',card);
        unloadlibrary(LIB);
        fprintf('WD_AI_CH_ChangeParam failed with error code %d\n',error);
...
%     end
    ...
    %chans = zeros(1,2,'uint16');
    %chans(1)=0;
    %chans(2)=1;
    %chansPtr = libpointer('int16Ptr', chans);
    %error = calllib(LIB,'WD_AI_ContReadMultiChannels',card,1,chansPtr,0,AI_ReadCount,ScanIntrv,SampIntrv,SyncMode);
    %WD_AI_ContScanChannels                          (U16 CardNumber, U16 Channel,U16 BufId, U32 ReadScans, U32 ScanIntrv, U32 SampIntrv, U16 SyncMode);
    error = calllib(LIB,'WD_AI_ContScanChannels',   card,               1,          0, AI_ReadCount/2, 1,1,SyncMode);
    if error < 0 
        calllib(LIB,'WD_AI_AsyncClear',card,0,AccessCnt);
        calllib(LIB,'WD_AI_ContBufferReset',card);
        calllib(LIB,'WD_Release_Card',card);
        unloadlibrary(LIB);
        fprintf('WD_AI_ContScanChannels failed with error code %d\n',error);
        return;
    end
    
    res.ReadCount = AI_ReadCount;
    res.pbuffer0 = pbuffer0;
    res.pbuffer1 = pbuffer1;
    res.card = card;
    res.LIB = LIB;
    res.SampleRate = SampleRate;
    res.AutoReset = AutoReset;
    res.AdRange = AdRange;
end