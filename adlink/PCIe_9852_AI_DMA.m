...
%-------------------------------------------------------------------------- 
%%
function res=PCIe_9852_AI_DMA(channel)
    clc
    %clear all;
    %close all;
...
%%
    ...
    %AI_ReadCount = uint32(10240000);
    %AI_ReadCount = uint32(15360000);
    %AI_ReadCount = uint32(16640000);
    %AI_ReadCount = uint32(17280000);
    %AI_ReadCount = uint32(17720000);
    AI_ReadCount = uint32(17825792); %17MB exact.
...
%     pbuffer.Value = buffer;
    pbuffer = calllib(LIB,'WD_Buffer_Alloc',card,AI_ReadCount*2);
    setdatatype(pbuffer,'uint16Ptr',1,AI_ReadCount);

    %pbuffer = libpointer('uint16Ptr',buffer);
    [error,tpbuffer,bufferID] = calllib(LIB,'WD_AI_ContBufferSetup',card,pbuffer,AI_ReadCount,bufferID);
    if error < 0
        calllib(LIB,'WD_Release_Card',card);
        unloadlibrary(LIB);
    ...
    error = calllib(LIB,'WD_AI_ContReadChannel',card,Channel,bufferID,AI_ReadCount,ScanIntrv,SampIntrv,SyncMode);
    if error < 0 
    ...
    %res = buffer; %%return value
    res = volts; %%return value
