...
%-------------------------------------------------------------------------- 
%%
function res=PCIe_9852_AI_DMA_DB_IN_2CH(channel, buffHeight, segmentSize, sync_ch)
    %clc %clear console
    %clear all;
    %close all;
...
%     clear d;
    ...
    if error < 0
        ...
        error = calllib(LIB,'WD_AI_CH_Config',card,1,AdRange);
    if error < 0
        calllib ('dasklib','WD_Release_Card',card);
        unloadlibrary(LIB);
        fprintf('WD_AI_CH_Config failed with error code %d\n',error);
    ...
    while TimeLeft>=0
        ...
        if HalfReady == true
            ...
            if channel == -1
                    disp('All Channels');
                    volts0 = volts(2:2:end);
                    volts1 = volts(1:2:end);
                    vsize = length(volts0);
            else
                vsize = length(volts);
            end
